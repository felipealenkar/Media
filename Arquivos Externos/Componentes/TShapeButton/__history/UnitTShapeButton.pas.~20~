...
type
  ...
  // Enumeração para direção do gradiente
  TGradientDirection = (gdNone, gdHorizontal, gdVertical, gdDiagonal1, gdDiagonal2, gdRadial);
  ...
  private
    ...
    FClickGradientStartColor: TColor;
    FClickGradientEndColor: TColor;
    ...
    procedure SetClickGradientStartColor(const Value: TColor);
    procedure SetClickGradientEndColor(const Value: TColor);
    ...
    // Método para criar brush de gradiente
    function CreateGradientBrush(const ARect: TGPRectF; StartColor, EndColor: TColor): TGPBrush;
    // Método para obter cores de gradiente baseado no estado atual
    procedure GetCurrentGradientColors(out StartColor, EndColor: TColor);
  protected
  ...
  published
    ...
    property GradientEndColor: TColor read FGradientEndColor write SetGradientEndColor default clBlue;
    property HoverGradientStartColor: TColor read FHoverGradientStartColor write SetHoverGradientStartColor default clYellow;
    property HoverGradientEndColor: TColor read FHoverGradientEndColor write SetHoverGradientEndColor default clRed;
    property ClickGradientStartColor: TColor read FClickGradientStartColor write SetClickGradientStartColor default clBlue;
    property ClickGradientEndColor: TColor read FClickGradientEndColor write SetClickGradientEndColor default clNavy;
...
begin
  ...
  FCaptionWordWrap := False;
  FImageIndex := -1;
  FImageAlignment := iaCenter;
  FImageMargin := 4;

  // Propriedades de gradiente com valores padrão
  FGradientDirection := gdNone;
  FGradientStartColor := clWhite;
  FGradientEndColor := clBlue;
  FHoverGradientStartColor := clYellow;
  FHoverGradientEndColor := clRed;
  FClickGradientStartColor := clBlue;
  FClickGradientEndColor := clNavy;
...
function TShapeButton.CreateGradientBrush(const ARect: TGPRectF; StartColor, EndColor: TColor): TGPBrush;
var
  StartPoint, EndPoint: TGPPointF;
  CenterX, CenterY, MaxRadius: Single;
begin
  case FGradientDirection of
    gdHorizontal:
      begin
        StartPoint.X := ARect.X;
        StartPoint.Y := ARect.Y;
        EndPoint.X := ARect.X + ARect.Width;
        EndPoint.Y := ARect.Y;
        Result := TGPLinearGradientBrush.Create(StartPoint, EndPoint,
          ColorToGDIPlusColor(StartColor), ColorToGDIPlusColor(EndColor));
      end;
    ...
    gdVertical:
      begin
        ...
        StartPoint.Y := ARect.Y;
        EndPoint.X := ARect.X;
        EndPoint.Y := ARect.Y + ARect.Height;
        Result := TGPLinearGradientBrush.Create(StartPoint, EndPoint,
          ColorToGDIPlusColor(StartColor), ColorToGDIPlusColor(EndColor));
      end;

    gdDiagonal1: // Diagonal esquerda-superior para direita-inferior
      begin
        StartPoint.X := ARect.X;
        StartPoint.Y := ARect.Y;
        EndPoint.X := ARect.X + ARect.Width;
    ...
    gdDiagonal2: // Diagonal direita-superior para esquerda-inferior
      begin
        StartPoint.X := ARect.X + ARect.Width;
        StartPoint.Y := ARect.Y;
        EndPoint.X := ARect.X;
        EndPoint.Y := ARect.Y + ARect.Height;
        Result := TGPLinearGradientBrush.Create(StartPoint, EndPoint,
    ...
    gdRadial:
      begin
        CenterX := ARect.X + ARect.Width / 2;
        CenterY := ARect.Y + ARect.Height / 2;
        MaxRadius := Max(ARect.Width, ARect.Height) / 2;

        // Para gradiente radial, vamos usar um LinearGradientBrush circular
        StartPoint.X := CenterX - MaxRadius;
        StartPoint.Y := CenterY;
        EndPoint.X := CenterX + MaxRadius;
        EndPoint.Y := CenterY;
      ...
      // gdNone - retorna brush sólido
      Result := TGPSolidBrush.Create(ColorToGDIPlusColor(StartColor));
  end;
end;

procedure TShapeButton.GetCurrentGradientColors(out StartColor, EndColor: TColor);
begin
  if not Enabled then
  begin
    StartColor := GetDisabledColor(FGradientStartColor);
    EndColor := GetDisabledColor(FGradientEndColor);
  end
  else if FIsMouseDown then
  begin
    StartColor := FClickGradientStartColor;
    EndColor := FClickGradientEndColor;
  end
  else if (Brush.Color = FHoverColor) then
  begin
    StartColor := FHoverGradientStartColor;
    EndColor := FHoverGradientEndColor;
  end
  else
  begin
    StartColor := FGradientStartColor;
    EndColor := FGradientEndColor;
  end;
end;
...
// Setters para propriedades de gradiente
procedure TShapeButton.SetGradientDirection(const Value: TGradientDirection);
begin
...
procedure TShapeButton.SetGradientStartColor(const Value: TColor);
begin
...
procedure TShapeButton.SetGradientEndColor(const Value: TColor);
begin
...
procedure TShapeButton.SetHoverGradientStartColor(const Value: TColor);
begin
  if FHoverGradientStartColor <> Value then
  begin
...
procedure TShapeButton.SetHoverGradientEndColor(const Value: TColor);
begin
  if FHoverGradientEndColor <> Value then
  begin
...
procedure TShapeButton.SetClickGradientStartColor(const Value: TColor);
begin
  if FClickGradientStartColor <> Value then
  begin
    FClickGradientStartColor := Value;
    Invalidate;
  end;
end;

procedure TShapeButton.SetClickGradientEndColor(const Value: TColor);
begin
  if FClickGradientEndColor <> Value then
  begin
    FClickGradientEndColor := Value;
    Invalidate;
  end;
end;

procedure TShapeButton.SetCaptionWordWrap(const Value: Boolean);
begin
...
var
  ...
  GradientStartColor, GradientEndColor: TColor;
  ...
  try
    ...
    try
      ...
      try
        ...
        // Obtém as cores do gradiente baseado no estado atual
        GetCurrentGradientColors(GradientStartColor, GradientEndColor);

        // Cria brush (gradiente ou sólido) e pen com as cores apropriadas
        if FGradientDirection <> gdNone then
          B := CreateGradientBrush(R, GradientStartColor, GradientEndColor)
        else
        ...
        begin
          ...
          P := TGPPen.Create(ColorToGDIPlusColor(PenColorToUse), W * 0.5);
        ...
        try
          ...
          // --- TEXTO (com WordWrap e inteligência para evitar sobreposição) ---
          if FCaption <> '' then
          begin
            ...
            try
              ...
              try
                ...
                try
                  // Configura WordWrap se ativado
                  if FCaptionWordWrap then
                    StringFormat.SetFormatFlags(0) // Permite quebra de linha
                  else
                    StringFormat.SetFormatFlags(StringFormatFlagsNoWrap); // Sem quebra de linha
                  ...
                  begin
                    AvailableTextArea := MakeRectF(4, 4, Width - 8, Height - 8); // Margem de 4px
                  end;
                  ...
                  // Define o retângulo final do texto
                  if FCaptionWordWrap then
                  begin
                    // Para WordWrap, usa toda a área disponível
                    TextRect := AvailableTextArea;
                    TextRect.Y := TextY;
                  end
                  else
                  begin
                    // Para texto sem wrap, usa as dimensões medidas
